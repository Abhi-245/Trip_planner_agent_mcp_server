...
#     api_key = "gnt4oU8DJM6Uunf4pSJ1OCkswUHJwW5W"
...
# # #     result = await get_hourly_weather("London")
# # #     print(json.dumps(result, indent=2))

# # # Run the test when executing directly
...
async def get_hourly_weather(location: str) -> Dict:
    try:
        ...
        location_key = get_cached_location_key(location)
        locations = None  # Ensure it's defined
        ...
        async with ClientSession() as session:
            ...
            if not locations:
                location_lookup_url = f"{base_url}/locations/v1/{location_key}"
                params = {"apikey": api_key}
                async with session.get(location_lookup_url, params=params) as response:
                    location_details = await response.json()
                    if response.status == 503:
                        raise Exception("❌ Weather API limit exceeded. Try again later or upgrade your plan.")
                    elif response.status != 200:
                        raise Exception(f"❌ Failed to fetch location details. HTTP {response.status}")
                    locations = [location_details]

            # Get current weather
            current_conditions_url = f"{base_url}/currentconditions/v1/{location_key}"
            async with session.get(current_conditions_url, params={"apikey": api_key}) as response:
                current_conditions = await response.json()
        ...
        raise Exception(f"⚠️ Weather fetch failed: {str(e)}")
